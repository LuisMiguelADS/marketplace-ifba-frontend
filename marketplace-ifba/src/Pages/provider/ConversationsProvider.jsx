...
const ConversationsProvider = () => {
    ...
    const { grupoPesquisa, user } = React.useContext(UserContext);
    const { request } = useFetch();
    const intervalRef = React.useRef(null);

    React.useEffect(() => {
        async function fetchProjetos() {
            if (!grupoPesquisa) return;
                ...
                try {
                    ...
                    if (response && response.ok) {
                        const projetosFiltrados = json.filter(projeto => 
                            projeto.grupoPesquisa?.idGrupoPesquisa === grupoPesquisa.idGrupoPesquisa
                        );
                        setProjetos(projetosFiltrados);
                        console.log('[CONVERSATIONSPROVIDER]: Sucesso na busca por projetos');
                    } else {
                        console.log('[CONVERSATIONSPROVIDER]: Falha na busca por projetos');
                    }
                } catch (error) {
                    console.error('[CONVERSATIONSPROVIDER]: Erro ao buscar projetos:', error);
                } finally {
                    setLoading(false);
                }
            }
    ...
    async function fetchChatMessages(chatId) {
        ...
        if (token && chatId) {
            try {
                ...
                const { response, json } = await request(url, options);
                if (response && response.ok) {
                    // CHAT_GET retorna o chat completo, então pegamos as mensagens
                    setChatMessages(json.mensagens || []);
                    console.log('[CONVERSATIONSPROVIDER]: Sucesso na busca por chat');
                } else {
                    console.log('[CONVERSATIONSPROVIDER]: Falha na busca por chat');
                    setChatMessages([]);
                }
            } catch (error) {
                console.error('[CONVERSATIONSPROVIDER]: Erro ao buscar chat:', error);
                setChatMessages([]);
            }
        }
    }
        ...
        <ContainerCards>
            {projetos.length > 0 ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px', width: '100%' }}>
                    Nenhum projeto encontrado para este grupo de pesquisa.
                </div>
            )}
        </ContainerCards>
